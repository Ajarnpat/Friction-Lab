...
- **Print this graph** prints the currently selected dataset on one A4 landscape page.
- The best-fit equation is drawn directly on the graph; regression statistics are not shown in print output.
- **Print completed graphs** places all datasets containing valid data on one A4 landscape page, in a maximum 2 × 3 layout. If fewer than six datasets have been entered, only those datasets are printed.
- Each mini-graph includes its own best-fit equation directly on the graph. Equation text is deliberately enlarged for readability in the six-graph A4 layout.
